...
bool matrix::operator== (matrix mat2){     // test if the two mat1 equals mat2
    matrix mat1(row,col,data);
    bool status = true;
...
bool matrix::operator!= (matrix mat2){ // test if mat1 does not equal mat2
    matrix mat1(row,col,data);
    bool status = false;
    if (mat1.col==mat2.col && mat1.row==mat2.row){
        for (int i=0; i<mat1.col*mat1.row; i++){
...
bool matrix::isSquare (){ // test if the matrix is square or not
    matrix mat(row,col,data);
    bool status = true;
    if (mat.row != mat.col){
        status = false;
...
matrix matrix::transpose(){    // return the transpose of the matrix
    matrix mat(row,col,data);
    matrix mat_trans;

int datatrans[mat.row*mat.col]={};
int x=0;
for (int i=0 ; i<mat.col; i++){
        for (int f=0; f<mat.row; f++){
    datatrans[x]= mat.data[(mat.row*f)+i];
    x++;
        }
}
createMatrix(mat.row, mat.col, datatrans, mat_trans);

return mat_trans;
}

bool matrix::isSymetric(){  // test if the matrix is symmetric
    matrix mat(row,col,data);
  bool status = true;
if (mat.row==mat.col){
...
bool matrix::isIdentity(){ // test if the matrix is an identity matrix
    matrix mat(row,col,data);
bool status = true;
if (mat.row==mat.col){
